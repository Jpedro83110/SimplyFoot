...
import {
  ...
  Alert, Platform, ScrollView, KeyboardAvoidingView, Keyboard, ActivityIndicator
} from 'react-native';
      ...
      if (selectedJoueurs && selectedJoueurs.length > 0) {
        ...
        if (participations.length > 0) {
          ...
          if (partError) {
            ...
            Alert.alert('Erreur', `Événement créé mais erreur participations: ${partError.message}`);
            return;
          }
      ...
      >
        ...
        <View style={styles.choicesContainer}>
          {TYPE_LABELS.map(opt => (
            ...
            >
              <Text style={type === opt.value ? styles.choiceSelectedText : styles.choiceText}>
                {opt.label}
              </Text>
            </TouchableOpacity>
          ))}
        ...
        <View style={styles.choicesContainer}>
          {equipes.map(eq => (
            <TouchableOpacity
              ...
              style={[
                styles.choiceButton,
                equipe === eq.id && styles.choiceSelected
              ]}
              onPress={() => setEquipe(eq.id)}
            >
              <Text style={equipe === eq.id ? styles.choiceSelectedText : styles.choiceText}>
        ...
        {loadingJoueurs ? (
          ...
          <>
            {joueursEquipe.length > 0 && (
              <TouchableOpacity style={styles.selectAllBtn} onPress={toggleAllJoueurs}>
            ...
            <View>
              {joueursEquipe.map(j => (
                ...
                >
                  <View style={[
                    styles.checkbox,
                    selectedJoueurs.includes(j.user_id) && styles.checkboxChecked  // Vérifier user_id
                  ]} />
                  <Text style={styles.joueurText}>{j.nom} {j.prenom}</Text>
                </TouchableOpacity>
              ))}
              {joueursEquipe.length === 0 && (
...
const styles = StyleSheet.create({
  ...
  input: {
    ...
    paddingVertical: 14, paddingHorizontal: 18, fontSize: 16,
    marginBottom: 10, borderWidth: 1, borderColor: '#333', width: '100%',
  },
  choicesContainer: {
    flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 10,
  },
  choiceButton: {
    backgroundColor: '#232323', borderRadius: 8, paddingVertical: 8, paddingHorizontal: 16,
    marginRight: 8, marginBottom: 8, borderWidth: 1, borderColor: '#333',
  },
  ...
  suggestionList: {
    backgroundColor: '#232323', borderRadius: 10, marginBottom: 10,
    borderWidth: 1, borderColor: '#00ff88', maxHeight: 200, overflow: 'hidden',
  },
  suggestionItem: {
    padding: 10, borderBottomColor: '#222', borderBottomWidth: 1,
  },
  ...
  buttonSticky: {
    backgroundColor: '#00ff88', paddingVertical: 18, borderRadius: 10, alignItems: 'center',
    position: 'absolute', bottom: 0, left: 0, right: 0, marginHorizontal: 0, marginBottom: 0,
  },
  ...
  checkbox: {
    width: 20, height: 20, borderRadius: 5, borderWidth: 2, borderColor: '#00ff88',
