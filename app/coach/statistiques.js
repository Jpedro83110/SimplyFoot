...
import {
  View, Text, StyleSheet, ScrollView, ActivityIndicator, Image, Pressable
} from 'react-native';
  ...
  async function fetchStats(forceRefresh = false) {
    try {
      ...
      for (let eq of equipes) {
        ...
        const moyMentale = (mentales && mentales.length)
          ? Math.round(mentales.reduce((a, e) => a + Number(e.note_globale || e.moyenne || 0), 0) / mentales.length)
          : 0;
        ...
        const moyTechnique = (techniques && techniques.length)
          ? Math.round(techniques.reduce((a, e) => a + Number(e.moyenne || 0), 0) / techniques.length)
          : 0;

        console.log('Moyenne technique calculée:', moyTechnique);

        // Moyenne globale
        const moyGlobale = moyMentale > 0 && moyTechnique > 0
        ...
        {stats.map((stat, idx) => (
          <View key={idx} style={styles.card}>
            ...
            <Text style={styles.debugInfo}>
              {stat.nbJoueurs} joueur(s) • {stat.nbJoueursEvalues} évalué(s) • {stat.nbEvalsMentales} éval(s) mentale • {stat.nbEvalsTechniques} éval(s) technique
            </Text>
