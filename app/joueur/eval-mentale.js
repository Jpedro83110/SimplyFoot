...
export default function EvalMentale() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        if (error && error.code !== 'PGRST116') throw new Error("Impossible de charger l'évaluation.");
  ...
  if (loading) return <ActivityIndicator size="large" color="#00ff88" style={{ marginTop: 40 }} />;
  if (error) return <Text style={{ color: 'red', margin: 30, textAlign: 'center' }}>{error}</Text>;
