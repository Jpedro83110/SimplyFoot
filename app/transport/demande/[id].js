...
if (connectedId) {
  ...
  async function proposerOuModifierTransport(autopick = false) {
    ...
    const exist = propositions.find(
      p => p.lieu_rdv === finalLieu && p.heure_rdv === finalHeure && p.parent_proposeur_id === userId
    );
    if (exist) {
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      {!loading && demande && (
        <View style={styles.card}>
          <Text style={styles.title}>
            <Ionicons name="football" color={YELLOW} size={18} /> {evenement?.titre || 'Évènement'}
          </Text>
          <Text style={styles.info}>
            <Ionicons name="calendar" color={GREEN} size={13} /> {evenement?.date} {evenement?.heure && `- ${evenement.heure}`}
          </Text>
          <Text style={styles.info}>
            <Ionicons name="person-circle" color={GREEN} size={15} /> {joueur?.prenom} {joueur?.nom}
          </Text>
          {decharge && (
            <Text style={styles.info}>
              <Ionicons name="people-circle" color={YELLOW} size={13} /> Parent : {decharge.parent_prenom} {decharge.parent_nom}
              {decharge.accepte_transport ? " (décharge acceptée)" : " (décharge non signée)"}
            </Text>
          )}
          <Text style={styles.info}>
            <Ionicons name="location" color={YELLOW} size={13} /> Adresse demandée : {demande.adresse_demande}
          </Text>
          <Text style={styles.info}>
            <Ionicons name="time" color={YELLOW} size={13} /> Heure demandée : {demande.heure_demande}
          </Text>
          <Text style={styles.statut}>
            <Ionicons name="alert-circle" color={YELLOW} size={15} /> Statut : <Text style={{ color: YELLOW }}>{demande.etat || demande.statut}</Text>
          </Text>
        </View>
      )}
      ...
      {!loading && demande && autorise && (
        <>
          <TouchableOpacity style={styles.actionBtn} onPress={() => { setEditPropId(null); proposerOuModifierTransport(true); }}>
            <Ionicons name="checkmark-circle" size={15} color="#111" />
            <Text style={styles.actionText}>Je le prends (lieu/heure demandés)</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.actionBtn, { backgroundColor: '#666' }]} onPress={() => { setEditPropId(null); setShowPropModal(true); }}>
      ...
      {!loading && demande && !autorise && (
        <Text style={{ color: "#ffe44d", margin: 18, fontStyle: "italic" }}>
          <Ionicons name="lock-closed" size={16} color="#ffe44d" /> Vous n'êtes pas autorisé à proposer ou valider un transport pour cette demande.
        </Text>
      )}
      ...
      {!loading && demande && (
        <>
          ...
          {(propositions.length === 0) && (
            <Text style={{ color: '#aaa', marginBottom: 8 }}>Aucune proposition reçue pour le moment.</Text>
          )}
            ...
            return (
              <View key={prop.id} style={styles.propositionCard}>
                <Text style={{ color: GREEN }}>
                  <Ionicons name="car-sport" size={13} color={GREEN} /> Par {prop.parent_proposeur?.prenom} {prop.parent_proposeur?.nom}
                </Text>
                <Text style={{ color: '#fff' }}>
                  <Ionicons name="location" size={11} color={YELLOW} /> Lieu : {prop.lieu_rdv}
                </Text>
                <Text style={{ color: '#fff' }}>
                  <Ionicons name="time" size={11} color={YELLOW} /> Heure : {prop.heure_rdv}
                </Text>

                {/* Boutons édition auteur */}
                {estAuteur && !signatureOk && (
                  <View style={{ flexDirection: 'row', gap: 10, marginVertical: 7 }}>
                    <TouchableOpacity
                ...
                {demande.joueur_id === userId && !prop.accepte && (
                  <TouchableOpacity style={styles.acceptBtn} onPress={() => accepterProposition(prop)}>
                    <Text style={styles.acceptText}>Valider ce transporteur</Text>
                  </TouchableOpacity>
                )}
                  ...
                  <>
                    {!signatureStatus.demandeur && (
                      <TouchableOpacity style={styles.acceptBtn}
                        ...
                        <Text style={styles.acceptText}>Je suis le parent du joueur, je signe</Text>
                      </TouchableOpacity>
                    )}
                    {!signatureStatus.conducteur && (
                      <TouchableOpacity style={styles.acceptBtn}
                        onPress={() => signer(prop, 'conducteur')}>
                        <Text style={styles.acceptText}>Je suis le conducteur, je signe</Text>
                      </TouchableOpacity>
                    )}
                    <Text style={{ color: '#0f0', marginTop: 7 }}>
                      {signatureStatus.demandeur && "✔️ Parent joueur signé  "}
      ...
      <Modal visible={showPropModal} animationType="slide" transparent>
        <View style={styles.modalBg}>
          <View style={styles.modalBox}>
            <Text style={styles.modalTitle}>{editPropId ? "Modifier la proposition" : "Proposer un autre lieu/heure"}</Text>
            <TextInput
            ...
            <TouchableOpacity style={styles.modalBtn}
              ...
              <Text style={styles.modalBtnText}>{editPropId ? "Modifier" : "Envoyer"}</Text>
            </TouchableOpacity>
            <TouchableOpacity style={[styles.modalBtn, { backgroundColor: '#444' }]}
              onPress={() => { setShowPropModal(false); setEditPropId(null); setLieu(''); setHeure(''); }}>
...
const styles = StyleSheet.create({
  ...
  actionBtn: { backgroundColor: GREEN, flexDirection: 'row', alignItems: 'center', borderRadius: 9, padding: 13, marginTop: 8, justifyContent: 'center' },
  actionText: { color: '#111', fontWeight: 'bold', marginLeft: 8 },
  acceptBtn: { backgroundColor: GREEN, borderRadius: 9, marginTop: 8, padding: 10, alignItems: 'center' },
