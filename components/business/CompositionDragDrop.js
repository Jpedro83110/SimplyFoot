...
export default function CompositionDragDrop({ evenementId }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                console.log(
                    '🎨 COMPOSITION: Participations récupérées:',
                ...
                console.log(
                    "🎨 COMPOSITION: Joueurs de l'équipe récupérés:",
                    allJoueurs?.length || 0,
                );
                ...
                if (joueursIds.length) {
                    ...
                    console.log(
                        '🎨 COMPOSITION: Utilisateurs récupérés:',
                ...
                console.log(
                    '🎨 COMPOSITION: Présents:',
                    presentsData.length,
                    presentsData.map((p) => p.nom),
                );
                console.log(
                    '🎨 COMPOSITION: Absents:',
                    absentsData.length,
                    absentsData.map((p) => p.nom),
                );
                console.log(
                    '🎨 COMPOSITION: Indécis:',
                    indecisData.length,
                    indecisData.map((p) => p.nom),
                );
                ...
                presentsData.forEach((j, i) => {
                    if (compo && compo.joueurs) {
                        ...
                        if (joueursSaved && joueursSaved[j.id]) {
                            ...
                            console.log(
                                `🎨 COMPOSITION: Position sauvegardée pour ${j.nom}:`,
                                joueursSaved[j.id],
                            );
                            return;
                        }
    ...
    const createPanResponder = (playerId) =>
        PanResponder.create({
            ...
            onPanResponderMove: positions[playerId]
                ? Animated.event([null, { dx: positions[playerId].x, dy: positions[playerId].y }], {
    ...
    const handleValider = async () => {
        ...
        console.log('🎨 COMPOSITION: Données:', {
            evenementId,
            coachId,
            equipeId,
        ...
        try {
            ...
            if (existing) {
                ...
                result = await supabase.from('compositions').insert({
            ...
            if (result.error) {
                Alert.alert(
                    'Erreur',
                    result.error.message ?? 'Échec de la sauvegarde de la composition.',
                );
