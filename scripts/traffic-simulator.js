...
class TrafficSimulator {
  ...
  async simulateGetDashboard(userClient) {
    return this.makeRequest(userClient, 'dashboard-joueur', async () => {
      const { data: user } = await userClient.client.from('utilisateurs').select('*').limit(1)
      const { data: joueur } = await userClient.client.from('joueurs').select('*').limit(1)
      const { data: events } = await userClient.client.from('evenements').select('*').limit(10)
  ...
  async simulateGetMessages(userClient) {
    return this.makeRequest(userClient, 'messages', async () => {
      const { data: prives } = await userClient.client.from('messages_prives').select('*').limit(50)
      const { data: groupes } = await userClient.client.from('messages_groupe_coach').select('*').limit(20)
  ...
  async simulateGetEvaluations(userClient) {
    return this.makeRequest(userClient, 'evaluations', async () => {
      const { data: mentales } = await userClient.client.from('evaluations_mentales').select('*').limit(10)
      const { data: techniques } = await userClient.client.from('evaluations_techniques').select('*').limit(10)
  ...
  async simulateGetDashboardCoach(userClient) {
    return this.makeRequest(userClient, 'dashboard-coach', async () => {
      ...
      const { data: events } = await userClient.client.from('evenements').select('*').limit(15)
    ...
    return this.makeRequest(userClient, 'update-composition', async () => {
      const { data } = await userClient.client
        ...
        .upsert({
          evenement_id: Math.floor(Math.random() * 50),
          joueur_id: Math.floor(Math.random() * 100),
          poste: ['Gardien', 'Défenseur', 'Milieu', 'Attaquant'][Math.floor(Math.random() * 4)]
        })
      return data
    })
  }
  ...
  async makeRequest(userClient, endpoint, requestFn) {
    ...
    try {
      ...
      this.metrics.errors++
      this.metrics.responseTimes.push(responseTime)
      this.metrics.errorsByType[error.message] = (this.metrics.errorsByType[error.message] || 0) + 1
  ...
  startMetricsReporting() {
    const interval = setInterval(() => {
      ...
      console.log(`📊 ${elapsed.toFixed(0)}s | Req: ${this.metrics.requests} | RPS: ${rps.toFixed(1)} | Erreurs: ${this.metrics.errors}`)
  ...
  generateReport() {
    ...
    const avgResponseTime = this.metrics.responseTimes.reduce((a, b) => a + b, 0) / this.metrics.responseTimes.length || 0
    ...
    const reportPath = path.join(process.cwd(), 'scripts', 'reports', `traffic-report-${Date.now()}.json`)
