...
class TestClubGenerator {
        ...
        for (let i = 0; i < count; i++) {
            ...
            const staffData = {
                ...
                date_naissance: faker.date
                    .between({ from: '1970-01-01', to: '1990-12-31' })
                    .toISOString()
                    .split('T')[0],
                diplome: Math.random() > 0.3,
                niveau_diplome:
                    Math.random() > 0.5
                        ? faker.helpers.arrayElement(['BEES', 'DES', 'BMF', 'CFF1', 'CFF2'])
                        : null,
        ...
        for (let teamIndex = 0; teamIndex < teams.length; teamIndex++) {
            ...
            for (let i = 0; i < playerCount; i++) {
                ...
                const playerData = {
                    ...
                    poste: faker.helpers.arrayElement([
                        'Gardien',
                        'Défenseur',
                        'Milieu',
                        'Attaquant',
                    ]),
                    numero_licence: `LIC${Math.floor(Math.random() * 999999)
                        .toString()
                        .padStart(6, '0')}`,
            ...
            for (let week = 0; week < 4; week++) {
                for (let training = 0; training < 2; training++) {
                    ...
                    const eventData = {
                        ...
                        lieu: faker.helpers.arrayElement([
                            'Terrain A',
                            'Terrain B',
                            'Gymnase',
                            'Stade Municipal',
                        ...
                        coach_id: coach.userId,
                        equipe_id: team.id,
                        meteo: faker.helpers.arrayElement([
                            'Ensoleillé',
                            'Nuageux',
                            'Pluvieux',
                            null,
        ...
        for (const match of matches) {
            ...
            if (teamPlayers.length > 0) {
                const selectedPlayers = faker.helpers.arrayElements(
                    teamPlayers,
                    Math.min(11, teamPlayers.length),
                );

                const composition = {};
                selectedPlayers.forEach((player, index) => {
                    composition[player.playerId] = faker.helpers.arrayElement([
                        'Gardien',
                        'Défenseur',
                        'Milieu',
                        'Attaquant',
    ...
    async generateClub(size = 'medium') {
        try {
            ...
            if (!config) {
                throw new Error(
                    `Taille de club invalide: ${size}. Utilisez: ${Object.keys(CLUB_CONFIGS).join(', ')}`,
                );
            }

            console.log(`🚀 Génération d'un club de taille "${size}"`);
            console.log(
                `📊 Configuration: ${config.teams} équipes, ${config.players} joueurs, ${config.coaches} coaches`,
            );
    ...
    async listTestClubs() {
        ...
        try {
            ...
            clubs.forEach((club, index) => {
                ...
                console.log(
                    `${index + 1}. ID: ${club.id} | ${club.nom} | Code: ${club.code_acces} | Créé: ${dateCreation}`,
                );
            });
            console.log('═'.repeat(80));
    ...
    async deleteClub(clubId) {
        ...
        try {
            ...
            const deletions = [
                {
                    table: 'participations_evenement',
                    condition: 'evenement_id',
                    name: 'participations',
                },
                { table: 'compositions', condition: 'evenement_id', name: 'compositions' },
                {
                    table: 'evaluations_mentales',
                    condition: 'coach_id',
                    name: 'évaluations mentales',
                },
                {
                    table: 'evaluations_techniques',
                    condition: 'coach_id',
                    name: 'évaluations techniques',
                },
                {
                    table: 'messages_prives',
                    condition: 'emetteur_id',
                    name: 'messages privés (expéditeur)',
                },
                {
                    table: 'messages_prives',
                    condition: 'recepteur_id',
                    name: 'messages privés (récepteur)',
                },
                {
                    table: 'messages_groupe_coach',
                    condition: 'expediteur_id',
                    name: 'messages groupe',
                },
                {
                    table: 'decharges_generales',
                    condition: 'joueur_id',
                    name: 'décharges parentales',
                },
                {
                    table: 'suivis_personnalises',
                    condition: 'coach_id',
                    name: 'suivis personnalisés',
                },
            ...
            for (const deletion of deletions) {
                try {
                    ...
                    if (
                        deletion.table === 'participations_evenement' ||
                        deletion.table === 'compositions'
                    ) {
                        ...
                        deletion.table === 'evaluations_mentales' ||
                        deletion.table === 'evaluations_techniques'
                    ) {
            ...
            console.log(
                `🗑️ "${stats.club.nom}" et toutes ses données ont été supprimés de la base`,
            );
        } catch (error) {
...
async function main() {
    ...
    switch (action) {
        ...
        default:
            ...
            console.log('\nUsage:');
            console.log(
                '  Créer un club:     node create-test-club.js create [small|medium|large|huge]',
            );
